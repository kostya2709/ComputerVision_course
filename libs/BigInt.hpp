...
BigInt BigIntpow(const BigInt& base, int exp) {
    if (exp < 0) {
        if (base == 0)
...
BigInt BigIntpow(const long long& base, int exp) {
    return BigIntpow(BigInt(base), exp);
}
...
BigInt BigIntpow(const std::string& base, int exp) {
    return BigIntpow(BigInt(base), exp);
